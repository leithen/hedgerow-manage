## ************************************************************
rm(list=ls())
cat('manage/functional_traits/lkm/bee.R\n')
source('src/misc.R')
library('bipartite')

traits <- read.csv('original/functional_traits/lkm/bee.csv',
...
## add Lecty, Excavate, and Sociality status for parasitic species
parasites <- which(traits$Sociality=='parasite')
traits$Excavate[parasites]  <- 'rent'
traits$Lecty[parasites]     <- 'parasite'
traits$Sociality[parasites] <- 'solitary'
...
write.csv(traits, file='my_data/functional_traits/bee.csv',
          row.names=TRUE)
write.csv(traits, file='~/Dropbox/hedgerow/data_sets/traditional/functional_traits/bee.csv', row.names=TRUE)
## ************************************************************
